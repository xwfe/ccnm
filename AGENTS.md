...
开始前依次阅读 [计划维护约定](docs/plan/README.md)、[当前状态](docs/plan/status.json) 和 [实施路线](docs/plan/ROADMAP.md) 中当前阶段。不要把聊天记录或 `.gld` 缓存当成最新实施状态。

## 工作规则

- 本地操作使用 `@gld`。先读 `git_status`、最近提交、`planning_manage state` 和 `task_manage status`；已有活动任务先确认归属，不另开任务覆盖。
- 状态以 Git 跟踪的 `docs/plan/status.json` 为准；`.gld` 只是本机镜像。开始、阻塞、交接、完成都要更新状态；没有证据不能标记完成。
