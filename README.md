...
**C51-01 已修（2026-09-25，P52）**：Runtime MCP 转接服务正常退出后子进程仍写工作区、写锁却已释放——P51 复现，P52 修复，macOS 与 Linux（Debian 13）都验过。仍够不着的是 server 派生的、离开进程组的后代（`setsid`、守护进程）：需要可靠单写交接的项目里，会这样做的 server 用 `[runtime_mcp] hidden` 藏掉或整个停用转接；停用转接也不等于普通命令已具备完整进程隔离。见 [P52 记录](docs/research/2026-09-25-p52-relay-group-cleanup.md)。
