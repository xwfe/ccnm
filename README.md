...
The current implementation runs the official Claude Code on an **Agent Node** and executes project tools on a **Runtime Node** through a persistent SSH stdio MCP transport. Source code, Git, builds, tests, and toolchains stay on the Runtime Node; AI credentials stay on the Agent Node.
...
> 当前处于发布前 dogfood 阶段。双机 macOS 链路已经真机跑通，但项目尚未正式发布，配置仍可能发生破坏性调整。
...
┌────────────────────────┐                ┌─────────────────────────┐
│ workspace / Git        │                │ Claude Code + OAuth     │
│ build / test / browser │◀── SSH stdio ─▶│ ccnm controller + tmux │
│ ccnm MCP runtime       │      MCP       │                         │
└────────────────────────┘                └─────────────────────────┘
```
...
受 ccnm 管理的 Claude 会话会关闭原生 Read/Edit/Write/Grep/Glob/Bash，让项目访问统一经过 Runtime Node。
