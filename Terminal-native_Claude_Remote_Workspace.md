...
**别名不够，还得带上 `ccnm_bin`（2026-09-05 修）。** 第一版只把别名传给
`start_from_work`，于是工作机这份 config 里的 `hosts.home.ccnm_bin` 被解析、被校验、然后被
忽略——家庭机把 ccnm 装在默认路径以外的人，会收到一句「command not found」，指的正是他自己
写对了的那个路径，而同一份 config 上别的命令全都正常。所以 `home_from_work()` 现在连 `Host`
一起返回：光有别名到不了一台机器，从来也没到过。失败分类也改成走 `ssh::classify`，跟其他每一次
远程调用同一个函数——127 说清它试的是哪个路径、去哪儿改，126 说 chmod，255 才是不可达。

### 两条链路怎么测的（2026-09-05）

`launcher.rs` 那组测试把**一端真实的输出喂进另一端真实的输入**，进程全是假的，消息全是真的。
其中一个走完整圈：家庭机发出的 `StartRequest` 由工作机那半段真的解开、真的握手、真的写出会话的
`mcp.json`，测试再从那个文件的 argv 里把 payload 解出来，比对第三跳打开的项目是不是第一跳说的
那个。

**为什么非连起来不可**：两端各自的单元测试都自己手搓消息，所以「两端各自都合理、但拿到的是
对方那个值」这类 bug 在里面永远不会出现。**加这组之前**试过：把 `start_interactive` 里的
`home_alias` 和 `work_ssh` 对调，当时那 369 个测试一个没红，会话连到错的机器上。四个字符串
（两个别名、两个二进制路径）故意写成互不相同，蒙对不了。

还有两处耦合编译器看不见，各补了一个 CLI 测试：工作机拼给家庭机的 `ccnm run <ws> --detached`
是个字符串字面量（改了 flag 名，断在对面，而报错讲的是参数、人看的是 workspace）；以及工作机
上的 attach / status / stop 必须本地作答——最想停掉一个会话的时候，往往正是链路出问题的时候。
