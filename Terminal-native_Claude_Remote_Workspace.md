...
每一跳原本都有单元测试，而每一个都自己手搓它想要的消息，所以「两端各自都合理、但拿到的是
对方那个值」这类 bug 在里面永远不会出现。**加连起来的测试之前**试过：把 `start_interactive` 里的
`home_alias` 和 `work_ssh` 对调，当时那 369 个测试一个没红，会话连到错的机器上。

所以补了两层，各管一段编译器看不见的耦合。怎么跑、各自盖到哪一步，写在
[development.md](docs/development.md#不用第二台机器能测到哪一步)，这里只说为什么是这两层：

- **库层**：一端真实的输出喂进另一端真实的输入，进程全假、消息全真。交互和 `--print` 各走一整圈，
  终点是会话的 `mcp.json`——Claude Code 会去跑的那个 ssh，模型碰项目唯一的路。
