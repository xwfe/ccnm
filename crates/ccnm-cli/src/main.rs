...
use ccnm_core::{Result, doctor, paths};
...
fn run(cli: Cli) -> Result<i32> {
    ...
    match cli.command {
        Command::Doctor { workspace } => {
            let report = doctor::run(&config_path, workspace.as_deref());
            print!("{}", report.render());
            Ok(report.exit_code())
        }
    }
...
/// Logs go to stderr so stdout stays parseable (doctor tables, hook JSON
/// later). Default level is warn; `-v` or `CCNM_LOG=debug` opens it up.
