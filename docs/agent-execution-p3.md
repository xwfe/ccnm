...
- `run`、`doctor`、`attach`、`status`、`result`、`stop` 使用同一选择。Agent Node 上发起的命令先把 `--agent` 原样作为受限标识转给 Runtime，由 Runtime 绑定 workspace/root/node，再由 Agent 本地 registry 解析 provider/profile。
...
- guard 文件位于 Runtime ccnm state，使用内核文件锁原子竞争。进程存活时不按超时转让。正常 shutdown 标记 released；进程异常消失后残留 held 记录视为 unknown，不能自动转让。P3 不承诺任意 shell 的 exactly-once、事务回滚或自动修复 stale guard。
...
- Claude legacy colocated 只在启动命令不再携带 remote-only `--tools ""`、`--mcp-config`/strict 限制后恢复候选支持；必须用真实 installed Claude 验证才列入支持矩阵。instance colocated 仍不开放。
