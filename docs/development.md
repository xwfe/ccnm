...
- **临时目录交给 `ccnm_testdir::TestDir`，别留给下一个人清。** 名字里带了 pid，上一次运行
  留下的目录就永远不会被下一次覆盖，每跑一次多一批：一次全量 `cargo test` 留 421 个，循环跑
  测试二进制排查偶发失败时，`$TMPDIR` 攒到过 18 万个、把盘写满。写法是
  `TestDir::adopt(你算好的路径)`，它离开作用域就删，测试失败时保留并把路径打到 stderr。
  **一个会咬人的地方**：`Store::open(&temp("x"))` 这样把守卫当临时值用，编译能过，但这条语句
  一结束目录就没了，测试接下来是在一个不存在的目录上跑——先 `let dir = temp("x");` 绑住。
  来龙去脉见 [P35 记录](research/p35-test-dir-cleanup-2026-09-17.md)。
