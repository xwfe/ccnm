...
Orchestrator 是**另一个仓库里的另一个产品**。ccnm 只交付状态归属边界、最小执行接口及适配器示例；外部产品是否建库、实现到哪一步，以它自己的仓库为准，本文不维护第二份状态。项目生命周期如何组合见[生命周期与职责](project-lifecycle.md)。
...
**ccnm 侧状态：协议与参考交接已提供，不代表下游产品已经实现。** 下游应在自己的仓库维护计划、授权和验收，不能由 ccnm 的阶段勾选替它标记完成。
