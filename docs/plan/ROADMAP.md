...
`P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7 → P8 → P9 → P10 → P11 → P12 → P13 → P14 → P15 → P16 → P17 → P18 → P19 → P20 → P21 → P22 → P23 → P24 → P25 → P26 → P27 → P28 → P29 → P30 → P31 → P32 → P33 → P34 → P35 → P36 → P37 → P38 → P39 → P40 → P41 → P42 → P43 → P44 → P45 → P46 → P48 → P49 → P50`（P47 被 2026-09-22 做完又整个撤销的一次占用过，不再用）。默认每轮只执行一个阶段。P0–P8 是 ccnm v1 收口和独立 Orchestrator 的接口交接；P9–P12 是 ccnm v1.x 的 Remote Workspace MCP 扩展；P13 是按真实 Host 行为修正两个入口共用的 instructions 投影；P21–P24 是 Codex 原生执行链；P25 修 P24 真机轮发现的预检错误码；P26 补原生链在 Runtime 侧的探活；P27 让 doctor 也探这条链；P28 让 CI 在声明的 rust-version 上编译一遍；P29 补测原生链的并发、在途请求与资源上限，P30 修它查出的 fs helper 活过放锁；P31 给 Runtime 保留输出加会话总量上限、结束即删和过期清理；P32 封存原生链（用户决定）；P33 把沙箱那项收益搬到两个入口共用的 `exec_command` 上；P34 修 `apply_patch` 日志锁探测靠关文件放锁、fork 窗口里漏拦的缺陷；P35 让测试建的临时目录跑完就删（纯测试代码）；P36–P41 是"工具面对齐原生能力"那条线（跨仓方案在 toexec 的 v3 计划）：P36 项目自带的 skills、P37 搜索模式与整文件覆盖与一行 shell、P38 让搜索的 glob 不越过 `.gitignore`、P39 view_image、P40 notebook 按 cell 读写、P41 后台命令；P42–P44 是跨仓重构评审 X04–X06 的三条收口：P42 后台命令的生命周期契约、P43 停不掉的命令不交出写权、P44 服务端自己验输入。P45 让 skill 的 frontmatter 读法对齐宿主（评审 X08）；P46 给受管会话的 Agent 自带功能加开关（v3 计划第 5 节第 6 步的离线部分）；P48 把两台机器上装好的 skills 交给会话（toexec v4 方案第 1 步）；P49 把 Runtime 上的 MCP server 转给会话（第 3 步）；P50 把 Agent 上的 MCP server 转给会话（第 4 步），顺带修了两个客户端会截掉 ccnm 大结果的问题。完整边界见 [双执行入口方案](runtime-surfaces.md)。

**P50 之后没有已排期的 ccnm 阶段。**用户 2026-09-22 要 gld / ccnm 用上两台机器上已经装好的 skills 和 MCP server，跨仓 [v4 方案](https://github.com/xwfe/toexec/blob/main/docs/plan/implementation-plan-v4-machine-skills-mcp.md) 里的四步都做完了：skills（P48）→ gld 聚合本机 MCP（gld RFC-0006）→ ccnm 在 Runtime 上代理 MCP（P49）→ ccnm 会话接 Agent 上的 MCP（P50）。v3 第 5 节第 6 步剩下的真实模型轮和第 7 步的对照实验要模型额度，要单独授权。
...
### P43 — 停不掉的命令不交出写权；写锁诊断指名道姓

**依赖 P42。对应跨仓评审 X05，以及[落地清单](../research/2026-09-19-cross-project-refactor-actions.md)的 C-B。**

现状是 2026-09-20 用真实二进制 + 中立客户端做故障注入跑出来的，不是读代码推的：

| 注入什么 | 现在会怎样 | 对不对 |
| --- | --- | --- |
| `mcp-serve` 被 `SIGKILL` | 锁文件留着 `held`，下一个会话被 `CCNM_E_POLICY` 拒掉、要人去看现场；它起的后台命令留在机器上 | 锁这一半**对**——保守拒绝正是要的。进程残留是 P41 已记的缺口 |
| 一个**停不掉**的后代（离开了进程组，又攥着管道不放） | `stop_all` 两段各等 10 秒后放弃，server **正常退出（退出码 0）**，写锁被标成 `released`，**下一个 coding 会话拿到了写权** | **错**。旧写者还在那棵树上，新写者已经进来——X05 验收第一句就是不许这样 |
| 同一棵工作树，两个不同的 `XDG_STATE_HOME` | 两个 coding 会话同时开着，各写各的文件，谁也不知道谁 | 是已知的设计边界（锁存在传入的 state 目录里），但没有一处文档说过，也没有任何发现手段 |

- **P43.1** 实测记录：上表的依据写进 research，包括脱离进程组那一路怎么造（fork 后 `setsid`、保留 stdout 管道）和它让 server 多花的 20.1 秒。
- **P43.2** 停不掉就不交权：`Jobs::stop_all` 改成返回还剩哪些命令（有 `output_ref` 的报 ref，还没拿到的报 `run #<id>`）；server 退出前只要还剩命令，就**不把写锁标成 `released`**——写成 `held … / abandoned …`，记下还剩几条、是哪几条。下一个会话因此被拒，和崩溃留下的一样要人来看，但话不一样：这不是崩溃，是 ccnm 明知有东西没停掉而**故意**不交权。
- **P43.3** 归属证据：`held` 那行记下 server 的 pid；acquire 拒绝时查一次那个 pid 现在是什么，据此分级说话——还在跑（指名道姓给 pid 和命令行）、已经不在（**仍然拒绝**，因为后代可能还在）、查不出来（拒绝）。**pid 不在从来不构成交权的理由**，它只让诊断说得准。旧格式（没有 pid 的）照旧能读、照旧拒绝。
- **P43.4** 文档：`operations.md` 的恢复流程按新诊断改；协议第 7 节 unknown 那格、`support-matrix`、`troubleshooting` 各一处；**把"同一棵树配两个 state home 就是两个互不知晓的写域"写进协议和运维手册**——它现在一个字都没有。
- **P43.5** 门禁同 P41.7。
...
### P46 — 受管会话默认开 WebSearch，其他 Agent 功能按工作区开

**依赖 P45。v3 计划第 5 节第 6 步的离线部分。用户 2026-09-22 定："默认开启 websearch，其他 Agent 功能是合理开关，按照你的建议设置"。** 零额度实测在 toexec 的 `evidence/v3-parity/agent-surface/`（`e0b5c7d`），ccnm 的记录在 [P46 记录](../research/p46-agent-tools-2026-09-22.md)。

为什么要做：远端受管会话到 P45 为止是 Claude `--tools ""`、Codex `web_search="disabled"`，模型查不了资料。放开之前先量清楚：`--tools` 认哪些名字（写错会被悄悄忽略）、子代理会不会绕过限制（不会）、`ToolSearch` 进白名单会把 ccnm 的工具推进延迟加载池、print 模式下没进允许表的 WebSearch/WebFetch 会被自动拒绝、Codex 的 `web_search` 在 CLI 默认模型上看不到效果。

- **P46.1** workspace 字段 `agent_tools`（`web_search` / `web_fetch` / `subagents` / `tasks`），写在 Runtime 上；不写 = `["web_search"]`，`[]` 全关，未知名字报错。
- **P46.2** 经 `ResolveReport` → `RunRequest` / `StartRequest` → session 记录传到 Agent，只在非默认值时写：默认配置的请求和以前逐字节相同，非默认值在旧 Agent 上按未知字段拒绝。协议版本号不动。
- **P46.3** Claude：`--tools` 只列开了的名字，settings 允许表加上它们，拒绝表加没开的以及 `NotebookEdit`、`Skill`；`ToolSearch` 永远不列。Codex：只接 `web_search`（开 = `cached`），子代理仍关。
- **P46.4** 两份实测 fixture 不重录：Claude 基线测试把有意改动逐条施加到期望值上，Codex argv 比对改用全关配置（录它时的配置），另加一条钉住默认只差 `web_search` 一个值。
...
### P48 — 两台机器上装好的 skills 交给会话

**依赖 P46。toexec v4 方案第 1 步。用户 2026-09-22 定："gld 或 ccnm 都可以使用 agent 和 runtime 机器上的已经安装的 skills 和 mcp……ccnm 默认全开……代码模块化，解耦，如果后期这块功能不需要，能简单的删除。抽取共用模块进 toexec"。** 零额度实测在 toexec 的 `evidence/v3-parity/machine-skills/`（`6741340`），ccnm 的记录在 [P48 记录](../research/p48-machine-skills-2026-09-22.md)。

为什么要做：P36 只交项目里的 skills；Runtime 执行账号装的不读，Agent 上装的被 P46 的拒绝表挡着，而 Codex 在远端会话里其实已经列出 Agent 上的 skills、却读不到（ccnm 关了它的 shell）。

- **P48.1** 实测原生怎么对待用户级 skills（Claude 放开 `Skill` / 只放 skills 目录的 `Read` / 同名谁赢；Codex 会不会列、能不能关），据此定 Agent 这边由 ccnm 自己提供。
- **P48.2** Runtime：`load_skill` 同时扫执行账号 HOME 的四个目录，项目的排前面，同名时装好的赢并说明被谁盖掉；符号链接和逐字节相同的拷贝只算一个；目录放不下全部名字时列放得下的那些。
- **P48.3** `load_skill` 加 `file` / `line`：只在 skill 自己目录里读（共享库 `toexec-skill` 0.3.0 的 `dir` 模块），64 KiB 一段。协议 fixture 手工改，不重录；`tools/list` 守住 16 KiB 预算。
- **P48.4** Agent：`ccnm internal agent-skills`，Claude 写进 `mcp.json` 并 allow，Codex 用 `-c mcp_servers.ccnm_agent.*`；Codex 远端会话一律 `skills.include_instructions=false`。
- **P48.5** 每台机器自己的 `[machine_skills]`（`enabled` 默认 true、`hidden`），不跨机器传，线上格式不变。
...
### P50 — Agent 机器上装好的 MCP server 交给会话

**依赖 P49。toexec v4 方案第 4 步。用户 2026-09-22："push, then continue step 4"；这一步当初定的是"只放网络类"，v4 总要求是 ccnm "默认全开"、不要了能整块删、共用代码进 toexec。** 零额度实测在 toexec 的 `evidence/v4-mcp/agent-mcp/`，ccnm 的记录在 [P50 记录](../research/p50-agent-mcp-2026-09-22.md)；共用机制 `toexec-mcp` 0.2.0（加 `kept`、`sse`，gld 同步改用）。

为什么要做：用户给 Claude Code / Codex 装的 MCP server（exa、DeepWiki、context7……）在 Agent 机器上，远端受管会话一个都用不上。

- **P50.1** 先量：把 server 直接写进原生客户端的配置，大结果会丢（Claude 超过约 5 万字符存盘只给 2 KB 预览，Codex 指定模型时只留约 12 KB）。据此由 ccnm 转：`ccnm internal agent-skills` 加 `call_mcp_tool`（和 Runtime 上的同名同用法）与 `read_mcp_result`（长结果留内存、32 KiB 一段）；一次调用的步骤和 P49 共用（`relay::call` + `Side`）。
- **P50.2** 默认给哪些：别的机器上的 HTTP 地址默认给，本机跑的（程序、`127.0.0.1`）要 Agent 的 `[agent_mcp] local` 点名；`hidden` 按名字藏；配置里自己关掉的不给（P49 那边也照此改）。workspace 的 `agent_tools` 加值 `mcp_servers`（默认开），去掉就不给。
- **P50.3** HTTP 经 Agent 上的 `curl`：地址和请求头写进只有本账号能读的文件交给 `curl -K`，SSE 拿到回复就停，401 报要登录；拆 SSE 用 `toexec-mcp` 的 `sse`。server 的环境去掉 Agent 登录变量和 `SSH_AUTH_SOCK`、加配置 `env`；会话结束停掉起过的 server。
- **P50.4** 附带修掉两处已有问题：Codex 会话加 `tool_output_token_limit=20000`（否则指定模型时 ccnm 每页都被砍到 12 KB）；`read_file`、`load_skill`、`call_mcp_tool` 带 `maxResultSizeChars`（否则 Claude 把 64 KiB 的一页存盘），`tools/list` 守住 16 KiB 预算。
- **P50.5** 会话文件：`session::create` 按 workspace 的 `agent_tools` 和 Agent 的 `[agent_mcp]` 决定 `ccnm_agent` 带不带 MCP 这一半，记录里写明它的工具，Claude 允许表、Codex `enabled_tools` 照写；P48 留下的会话文件照旧读。配置说明、使用说明、协议第 5 节、支持矩阵、README 跟着改。
- **P50.6** Rust 单元测试、中立 MCP 客户端 `tests/test_agent_mcp.py`、真实 Claude Code 2.1.278 / Codex 0.154.0 与 0.155.1 接真实 Agent 端服务（零额度），真实 DeepWiki 经 `curl` 读全。门禁同 P37.6，外加 `check_protocol`。

停止点：不把 server 写进原生客户端的配置；不转老的 HTTP+SSE 传输、不转 server 的 resources / prompts；不跑真实模型；gld 那一半（改用 `kept` / `sse`）记在 gld 自己的仓库；不发版。

### P51 — v0.9.0 完成度、生命周期与文档审计

**依赖 P50。用户 2026-09-23 要求：核对当前完成情况和文档要求，评估完整开发生命周期的缺口，同步 README 与相关文档。**

- **P51.1** 以当前 Git、代码和已提交证据核对能力与限制；区分历史阶段完成、当前实现、离线验证和真实环境验收，记录缺陷及待复验项。
- **P51.2** 建立项目生命周期覆盖与职责矩阵，给出有依赖和验收条件的后续优先级；不把编排、发布授权或业务验收塞进 Runtime。
- **P51.3** 同步 README、架构、安全、使用、支持矩阵、协议入口及计划交接；保留历史证据，修正过期的当前声明。
- **P51.4** 执行文档与协议检查、相关回归测试和 diff 检查，记录真实结果与未覆盖范围，提交本轮文档。

停止点：仅审计与文档同步，不修改产品行为、不调用付费模型、不修改系统权限或登录、不部署、不推送、不发版；发现代码缺陷列入后续工作，不用文档修订冒充修复。

### P52 — MCP relay 可靠收尾与写权交接

**依赖 P51。待实施：修复 P51 已复现的 C51-01，不增加新的产品功能。**

- **P52.1** 把 P51 隔离探针转成中立客户端回归；固定 leader 正常退出、同组子进程仍写、guard released、第二 writer 可进入的失败证据。
- **P52.2** 为 relay 收尾提供可判定的结果，Runtime 在后代归属或清理结果未知时不得安全放锁；不得信号无关或 pid 已重用的进程。
- **P52.3** 覆盖 EOF、leader 先退出、后代关闭管道/脱组、取消、超时、强杀及闲置回收；同步复核 Agent 本机 relay 的共享关闭路径，不把 Runtime 复现冒充 Agent 验收。
- **P52.4** macOS 与 Linux 的相应回归及文档/协议门禁通过；标明 OS 监督边界和未覆盖项，更新支持矩阵、安全/运维/协议披露后才关闭 C51-01。

停止点：机制抽取遵守 toexec 与 ccnm 的职责分离，跨仓修改、依赖发布、SSH 真机、模型额度和部署分别确认范围；不重开 exec-server、不做任务编排、不把普通进程组清理升级成未证明的 sandbox 承诺。随后优先将 Python/计划/协议检查接入 CI（C51-03），再做真实项目的生命周期验收。

### P53 — Python、计划与协议 CI 门禁

**依赖 P52。将 P51 已提出的 C51-03 正式编号，不增加移动产品功能。** 后续手机 SSH 与网页终端的共同前提见[移动端实施总纲](mobile-access.md)。

- **P53.1** 核对 CI/release 的真实构建顺序和平台前提，登记 Python 中立客户端所需的 debug 二进制、工具与可解释的平台 skip；不能靠缓存或整套跳过制造成功。
- **P53.2** 在适用的 macOS/Linux job 接入 `check_plan`、`check_protocol` 和 Python 全套可执行测试；发布/上传必须依赖相应检查成功。
- **P53.3** 有通过与失败阻断的验证证据，确认门禁失败不能进入发布步骤；本地配置/脚本验证与真实 GitHub runner 结果分开记录，缺后者不写成线上通过，缺运行授权则记录阻塞。
- **P53.4** 同步开发/发布与支持说明、C51-03 状态；计划检查、相关回归和 diff 检查通过。

停止点：本阶段不修改移动入口或运行真实模型；不为了验证工作流擅自 push、创建 tag 或发布，所需线上执行另行授权。完成后下一轮才开始 P54。

### P54 — 手机 SSH 操作 hpsrv Runtime

**依赖 P53。用户 2026-09-24 选择方案一；细化任务见[手机 SSH 计划](mobile-ssh.md)。** 复用常在线 macOS Agent 与 hpsrv/ccrun，不实现 Linux Controller 或移动 App。

- **P54.1** 完成获准范围内的只读基线核查：角色、实际 UID、Controller/登录、两端含 P52 修复的构建、SSH 认证方式、网络及 Runtime 安全/工具链；不以历史 hpsrv 证据代替当前部署。
- **P54.2** 形成手机 SSH 使用/部署/回退手册和分项授权清单；设备独立 key、主机指纹、禁止凭据转发，普通 SSH 与 Tailscale SSH 不混淆。
- **P54.3** 文档、计划与新增 helper（如确有必要）的离线测试通过；不为这条路径强造新程序或修改 ccnm 公共契约。
- **P54.4** 授权后在真实手机蜂窝网络上按 SSH-01 至 SSH-03 完成启动/精确 attach/输入/审批与 hpsrv 小项目开发闭环，保留实际 provider、属主、diff 和独立测试证据。
- **P54.5** SSH-04 至 SSH-11 的接力、断线、MCP 故障、拒绝、幂等 stop、撤权及恢复逐项记录；明确未验平台与非目标，更新用户手册、运维、排错和支持矩阵。

停止点：真机写入、模型额度、SSH/网络规则及系统变更须分项授权；缺失时 blocked。不得用关闭 `[runtime_mcp]` 顶替 P52，不自动安装 ttyd 或进入 P55。

### P55 — attach-only 浏览器终端离线适配

**依赖 P54。方案二的仓库实现阶段，见[浏览器终端计划](mobile-web-terminal.md)。** 只复用公共 `ccnm attach`，不新建 Web Agent launcher 或任务控制 API。

- **P55.1** 定义 Operator 管理的无凭据目标绑定，固定 workspace/instance/ccnm session ID，验证配置/属主/权限/路径与运行 UID；不依赖私有 session 文件格式。
- **P55.2** 实现最小 attach helper：参数数组直接 exec、失败即退出、无 shell/run/new-session/最新会话回退；断开只收回 attach 客户端，换绑与输入不自动重放。
- **P55.3** 提供仅 loopback、可写、Origin 校验、可信代理认证 header、单 Web 客户端的 ttyd 参数模板及独立 macOS LaunchAgent 模板；说明终端不是 sandbox、header 不是白名单、同机进程信任边界。
- **P55.4** 离线覆盖参数注入、错误配置与会话、重复连接、PTY/信号、模板冲突和回退；文档/计划/相关脚本测试通过，交付候选安装清单。

停止点：不安装依赖、不 bootstrap 服务、不启动真实 Agent、不配置 Serve/HTTPS、不改 tailnet 或公有协议；测试只用隔离 fake/mock，不能冒充真实手机验收。完成后停止。

### P56 — Serve 私网部署与手机联合验收

**依赖 P55。方案二的授权部署阶段，见[浏览器终端计划](mobile-web-terminal.md)。** SSH 作为独立救援入口保留；没有明确系统与网络授权不执行部署。

- **P56.1** 复核 P54 基线及现有 Serve/Funnel/监听/许可，记录具体安装变体与版本，列出身份、资源、HTTPS/网络/模型授权与逐项回退；本机不可信多用户边界未解决时拒绝部署。
- **P56.2** 在 Agent 以 Operator 身份部署受控 helper/ttyd，先验证 loopback 与拒绝路径，再开放已受限的专用 HTTPS Serve 路由；不使用 Funnel、裸 shell 或全局重置。
- **P56.3** WEB-01 至 WEB-05：真实手机蜂窝网络、IME/控制键/审批、重复断线、SSH/Web 接力与 hpsrv 项目闭环；绑定同一会话，不产生新 Agent。
- **P56.4** WEB-06 至 WEB-09、WEB-12：未授权/撤权设备、WebSocket、Origin/header/URL 注入、后端暴露、失效会话与换绑逐项验证；新连接拒绝与既有连接终止分别证明。
- **P56.5** WEB-10、WEB-11：入口停止/重启不误杀 ccnm，会话 MCP 故障按收尾与写权契约处理；普通命令与已修复 relay 都有相应证据。
- **P56.6** 完成精准撤入口/回退与清理检查，更新用户手册、支持矩阵、运维和排错，保存手机/浏览器/provider/两端构建与未验范围；不能以桌面网页或 mock 代替手机通过。

停止点：只声明具名环境的移动终端能力，不承诺跨重启无人值守；不新增项目预览代理、长期服务托管、多租户或聊天 UI，不自动推广、push 或发版。
