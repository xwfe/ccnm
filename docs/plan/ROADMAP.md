...
`P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7 → P8 → P9 → P10 → P11 → P12 → P13 → P14 → P15 → P16 → P17 → P18 → P19 → P20 → P21 → P22 → P23 → P24`。默认每轮只执行一个阶段。P0–P8 是 ccnm v1 收口和独立 Orchestrator 的接口交接；P9–P12 是 ccnm v1.x 的 Remote Workspace MCP 扩展；P13 是按真实 Host 行为修正两个入口共用的 instructions 投影；P21–P24 是 Codex 原生执行链；P27 让 doctor 也探这条链（P25、P26 在另外两个未合并的分支上，见 P27 的编号说明）。完整边界见 [双执行入口方案](runtime-surfaces.md)。
...
**实际只依赖 P24。**（编号说明：P24 之后另有两个分支同时立项，分支里都记作 P25——「MCP 握手失败时保留远端报的错误码」（57658de）和「原生链 Runtime 侧探活与无响应超时」（4dae491）。本阶段开工在它们之后，按开工先后记为 P27；本分支里没有 P25、P26，`depends_on` 暂写 P24，合并时改成前一阶段。）P24 真机轮发现（[记录](../research/p24-native-real-machine-2026-09-16.md)第九节）：`codex_exec_server = true` 的 workspace，`ccnm doctor` 只做 MCP 握手；Runtime 节点没配 `codex_bin`、Codex 版本不是 0.154.0、exec-server 起不来，都要到 `ccnm run` 的预检（`native_runtime_preflight`：一次空的 `exec-serve` 会话，stdin 立刻关闭）才报出来。
