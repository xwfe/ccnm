...
> **这一轮已于 2026-09-11 执行完毕，结果见[真机记录](../research/p12-real-project-2026-09-11.md)。** 下面保留原计划，因为它仍是重跑同一轮的操作说明。重跑前先看这四处更正：
>
> 1. **Host 不是本机。** 本机的 `claude` CLI 没有登录（桌面版的凭据 CLI 拿不到），所以真实 Host 那一腿在 `fodelf` 上跑，它有 Claude Code 2.1.268 且登录有效。代价是要给 fodelf 一把**自己的**一次性密钥（私钥不离开它），公钥按 `appended-line-fodelf` 记进同一个 root 清单。
> 2. **客户端配置的 `this` 必须指向一个真实的 `[nodes.*]` 条目**，否则 `CCNM_E_CONFIG`；而 `ccnm_bin` 那一行**不能照抄默认值**（默认 `~/.local/bin/ccnm` 不是绝对路径，校验会拒），省掉即可。
> 3. **`nodejs.org` 在这台 Runtime 的出口上会被 TLS reset**，`static.rust-lang.org` 不会。工具链脚本因此支持 `--node-base-url` 换镜像加 `--node-sha256` 固定官方哈希——哈希在能连上官方的那台机器上取。
> 4. **这一轮的公钥在收尾时删掉了**（私钥已销毁，留着公钥等于装了一把没人持有私钥的钥匙）。账号和工具链按用户选择保留，所以重跑时只需要装新密钥、重建 workspace 和项目副本。
