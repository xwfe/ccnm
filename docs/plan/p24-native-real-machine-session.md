...
> **这一轮已于 2026-09-16 执行完毕**：用户同意了 A0–A9，root 步骤由我执行，收尾按第七节做完。结果、和本计划不一样的四处（A3/A4 改为本机下载或打包后 scp、A7 由我经 `launchctl bootstrap` 起临时 Controller、A9 只丢一条连接）以及清理核对，见[真机记录](../research/p24-native-real-machine-2026-09-16.md)。下面保留原计划，作为重跑同一轮的操作说明。
>
> 原状态（2026-09-16 22:16）：只列了清单，没有做任何变更。第四节每一条都要用户明确同意才做；同意其中一条不等于同意别的。第三节的现场事实是本机和 hpsrv 上的只读查询得来的。
