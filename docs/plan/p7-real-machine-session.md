...
这份计划已经按第四节的顺序执行到第 5 步，**结果不在这里，在[真机记录](../research/p7-real-machine-2026-09-10.md)**：

| 步骤 | 状态 |
| --- | --- |
| 0 只读预检、1 建环境、2 部署 | 做完 |
| 3 对照（原 P6.3） | Claude 通过；**Codex 没跑**，装在本机且没额度，要相反拓扑，等额度 |
| 4 完整 dogfood | 通过（ccnm 自己的克隆，两个会话，产出的两个 commit 已合进 main） |
| 5 门禁 | 通过 |
| 6 清理 | **没做**：环境留着等 Codex，撤销脚本和逆序清单仍然有效 |

第三节"一个方向够不够"的假设，在只读预检里确认了：两个 provider 的 CLI 确实不在同一台，所以 Claude 只能是 fodelf 当 Agent，而 Codex 只能反过来——**一个方向不够，但另一个方向要等额度**。
