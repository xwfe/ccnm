...
写互斥只在同一 state 目录及相同 canonical Git common-dir / 非 Git workspace 资源范围内有效，不拦普通编辑器或旁路命令。Runtime MCP 转接的“旧子进程仍在写却交出写权”（C51-01）P52 已修：同组后代交权前清掉并确认，但离开进程组的后代看不见，所以 `released` 仍不是整棵进程树的证明，见 [P52 记录](research/2026-09-25-p52-relay-group-cleanup.md)。
