...
服务端**按顺序**处理请求：读一行、答一行、再读下一行。没有并发，响应顺序就是请求顺序。

这带来一条客户端必须守的规矩：**别只管写不管读**。管道缓冲是有限的（通常 64 KiB），你连发一堆请求却不收响应，缓冲很快填满，服务端阻塞在写、于是不再读你的 stdin，你也阻塞在写——两边一起卡死，谁也不会超时。

两种安全的写法：

- **一问一答**：写一条，读一条，再写下一条。最简单，够用。
- **并发读写**：一个线程（或一个 async 任务）专门读响应，另一个写请求。想流水线发请求就得这么做。

长任务不需要流水线：`session.start` 立刻返回，之后轮询就行。
...
**`-32008` 目前没有实现会返回。** 这个码在契约里留着，但当前 build 从不发：工作树的写入 guard 由 Runtime 侧的 MCP 进程在会话跑起来之后才获取，`session.start` 那一刻不检查。工作树被占的实际表现是会话起来了然后失败，不是启动被拒。当前实现与契约的全部差距见[协议说明](README.md)。
...
**`ccnm.machine/1` 于 2026-09-10 冻结**，从这一刻起下面的兼容规则是承诺，不再是"定稿后要遵守的参照"。

冻结的依据是两个 provider 各跑通过一次真机双机闭环，并与人类 CLI 对同一件事做副作用对照——两条腿的产物属主相同、`usage` 端到端到达调用方：[Claude](../research/p7-real-machine-2026-09-10.md)、[Codex](../research/p7-codex-parity-2026-09-10.md)。冻结前最后改掉的两处行为记在 [P7.5 记录](../research/p7-5-freeze-2026-09-10.md)里，其中 `ccnm stop` 的幂等修正也影响本协议的 `session.stop`：那个方法本来就对终态记录幂等，现在它底下的执行路径也是了。

**冻结的是这份契约，不是产品的全部能力。** 没实现的仍然没实现（见[协议说明](README.md)的"实现得比契约少"清单），未来补上时按下面的加法规则走。
...
- 加一个新的**可选**响应字段。
- 加一个新的**可选**参数。
- 加一个新的 `capabilities` 键。
- 在 `-32000..=-32099` 里加一个新错误码。
- 加一个新的**非终态** `state`。

### 服务端不能做的（要升到 `ccnm.machine/2`）

- 删字段，或者把可选字段变成必填。
- 改一个字段的含义，哪怕名字没变。
- 改一个错误码的语义，或者把某个情况改用别的码。
- 收紧已有参数的取值范围（比如原来接受的值开始被拒）。
- **加一个新的终态。** 见下。

### 客户端必须做的

- **忽略不认识的字段。** 响应里多出来的键一律跳过，不要当成错误。
- **忽略不认识的 `capabilities` 键。** 反过来，键不在就是没有那项能力。
- **按 `code` 分支，不要解析 `message`。** message 是给人看的，措辞会变。
- **把不认识的 `state` 当作"还没结束"**，继续轮询。

最后一条是有代价的承诺换来的：**终态集合冻结在 `completed` / `failed` / `unknown` 三个**，v1 之内不会再加。所以"不认识就继续等"永远不会让客户端错过一次结束——新加的状态一定是过程中的状态，等下去总会走到这三个之一。

反过来，如果哪天真的需要第四个终态，那就是破坏性变更，走 `ccnm.machine/2`。
