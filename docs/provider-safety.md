...
还有第三个开关 `allow_unattended_exec`，**它不在这张表里，因为它不授权任何东西**：它只决定交互式会话执行 `exec_command` 前要不要问人一次，命令本身能做什么完全不变。见[支持矩阵](support-matrix.md#三个逃生开关接受了什么以及绝不会变绿)。
