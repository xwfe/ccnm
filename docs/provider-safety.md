...
| --- | --- | --- | --- |
| Agent 私有目录与认证 | Agent 的本地配置、OS 执行身份、官方 CLI | 检查目录/file metadata；官方 CLI 独立登录与报告状态 | 读取认证内容、从 Runtime 接收私有路径、复制/代理订阅凭据 |
| Agent → SSH 环境 | Agent 进程 | 清除所有已知 Provider 的私有环境和其他凭据形态的变量；本机 SSH 可使用自己的认证 agent | 转发认证 agent、继承未知安全策略的 ControlMaster、通过 SendEnv 下发 Agent 环境 |
...
Runtime 在 exec 前复查凭据可访问性和环境，不能仅依赖 session 启动时的旧审计。即使 `allow_unconfined_exec=true`，已知 Agent 凭据可访问/未知、来源不明认证环境仍拒绝 exec；这个选项只能接受既有账号 confinement 风险，不能授权共享 Agent 登录。OS 权限仍须阻止进程在检查后获得新权限：审计不是无竞态 sandbox。

## 分层验收与兼容

1. Agent transport 和预检 SSH：分别检查构造命令、子进程实际收到的合成环境及 OpenSSH 解析选项。不得借 Runtime 的 `env -i` 证明上游已清理。
