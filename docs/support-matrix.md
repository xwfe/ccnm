...
| Runtime MCP relay（P49） | stdio 转接已实现，有本机测试及零额度 Host 证据 | 同组后代 P52 起在交权前清掉并确认，清不掉不交权（macOS、Linux 已验）；离开进程组的后代够不着；仍缺完整真实模型组合验收 |
...
**C51-01**（relay server 退出后同组子进程仍写、写锁却交出）：P52 已修并关闭，macOS 与 Linux（hpsrv Debian 13，`ccrun`）都有正反两面的证据，见 [P52 记录](research/2026-09-25-p52-relay-group-cleanup.md)。仍够不着的是 server 派生的、离开进程组的后代（`setsid`、守护进程）：要求可靠单写交接时，这类 server 用 `[runtime_mcp] hidden` 藏掉或停用整个 `[runtime_mcp]`，关闭现有会话并核实旧进程树；仅改配置不能清理旧进程，删除 guard 不能证明可以安全重开。
...
| 项目自带的 skills（`load_skill` 工具 + MCP prompts） | 支持，**只有离线证据** | 三种入口（受管 Claude/Codex、外部 MCP 的 `read` 与 `coding`）共用一个实现（[使用说明](usage.md#项目自带的-skills)、[协议第 5.1 节](protocol/remote-workspace-mcp-v1.md#51-load_skill-与-prompts项目自带的-skillsp36-新增)、[P36 记录](research/p36-skills-surface-2026-09-17.md)）。**验过的**：发现三个位置、重名与读不了的文件给出原因、经 symlink 出 workspace 的 skill 被拒、目录不超过 2048 个 UTF-16 码元、参数替换对齐 Claude Code 2.1.273 的实际行为、`` !`命令` `` 不被执行、prompts 列出与获取——Rust 单元测试加一个不 import ccnm 代码的中立 MCP 客户端对真实二进制跑。frontmatter 解析器在一台开发机的 810 个真实 skill / 命令文件上全部读得出来。通道选择依据是对 Claude Code 2.1.273 和 Codex 0.154.0 的零额度实测（前者每个工具的 description 留 2048 码元、认 prompts；后者不截、只调 `tools/list`）。**真机验过了**（2026-09-20，[记录](research/real-machine-p36-p44-2026-09-20.md)）：给这个仓库临时放一个写着三步门禁的 `SKILL.md`，只对模型说"跑一遍这个项目的提交门禁"，它跑出来的三步内容和顺序与 skill 一字不差（其中 `scripts/check_plan.py` 是本项目专有，不读 skill 猜不出来）——**模型会因为目录里的描述主动去加载 skill**。**没验的**：Claude Code 里 `/mcp__ccnm__<名字>` 的交互没有人工点过；Codex 在 Code Mode 下看到的是嵌套注册表里的工具，`load_skill` 的 description 在那条路径上有没有完整到模型面前没有单独量。**故意不做的**：`` !`命令` `` 自动执行、`allowed-tools` / `hooks` 等 frontmatter、MCP 官方 skills 扩展（SEP-2640，另一个阶段）。（执行账号 HOME 下装好的 skills 原来也在这一栏，P48 做了，见下一行。） |
| 两台机器上装好的 skills（Runtime 执行账号的并进 `load_skill`，Agent 上的经 `ccnm internal agent-skills`；`load_skill` 的 `file` / `line`） | 支持，**只有零额度证据** | 默认全开，每台机器用自己的 `[machine_skills]` 关或按名字藏（[配置说明](configuration.md#machine_skills)、[协议第 5.1 节](protocol/remote-workspace-mcp-v1.md#51-load_skill-与-prompts项目自带的-skillsp36-新增)、[P48 记录](research/p48-machine-skills-2026-09-22.md)）。**验过的**：零额度实测 Claude Code 2.1.278 与 Codex 0.154.0 原生怎么对待用户级 skills（决定了 Agent 这边由 ccnm 自己提供，见记录）；Rust 单元测试（同名时装好的赢且说明被谁盖掉、藏掉的和没装一样、符号链接装的只算一个、只在 skill 自己目录里读且拒绝点文件、长文件分段接得上、超过 100 个先丢装好的、目录不超 2048 码元）；中立 MCP 客户端对真实二进制跑 Runtime 和 Agent 两个服务；**真实 Claude Code 2.1.278 与 Codex 0.154.0（`gpt-5.1-codex`）接上真实的 Agent 端服务**（假模型，零额度）：工具出现在工具表、print 模式不被权限拦、读得到附件、`.env` 被拒，Codex 自己那份清单消失。**没验的**：真实模型会不会主动用装好的 skill；Codex 不写 `model`（Code Mode）时经 JS 调 `ccnm_agent` 这条路；Linux；gld hub 经 `remote_load_skill` 读远端装好的 skill 的附件（gld 那边加了参数透传，没跑端到端）。 |
| 项目那台机器上的 MCP server（`call_mcp_tool`：项目 `.mcp.json` 里的、执行账号装的） | 支持，**只有零额度证据** | 默认全开，Runtime 用自己的 `[runtime_mcp]` 关、不读项目的、按名字藏（[使用说明](usage.md#项目那台机器上的-mcp-server)、[配置说明](configuration.md#runtime_mcp)、[协议第 5.7 节](protocol/remote-workspace-mcp-v1.md#57-call_mcp_toolruntime-上的-mcp-serverp49-新增)、[P49 记录](research/p49-runtime-mcp-2026-09-22.md)）。**验过的**：Rust 单元测试（只有能转的才进说明、HTTP 和缺变量的写明原因、配置里的 token 到了而 Agent 的登录没到、大结果进留存目录、关 / 藏 / 不读项目、只有能写且有 server 的会话才列、有人值守时要问人）；中立 MCP 客户端对真实二进制（coding 会话调通、大结果经 `read_output` 读全 52 000 字节、read 会话没有也调不到、执行门挡住起 server 但不挡列清单、会话结束时 server 被停掉而下一个 coding 会话进得来、server 留在自己进程组里的子进程也在交权前没了（P52）；这个文件 P52 时也在 Debian 13 / x86_64 上以 `ccrun` 全过）；gld 连真实 ccnm 的组合测试；**真实 Claude Code 2.1.278 与 Codex 0.155.1（`gpt-5.1-codex`）经真实 ccnm 调通**（假模型，零额度，toexec `evidence/v4-mcp/runtime-relay/`）。**没验的**：真实模型会不会主动用；受管会话经 SSH 那条路和有人值守时的询问在真实 Host 上的样子；Codex 0.154.0（受管会话钉的版本）；Linux 上的真实 Host；OS 沙箱里跑 server。**故意不做的**：HTTP server（Agent 上装的由下一行转）、server 的 resources / prompts。 |
| Agent 机器上装好的 MCP server（`ccnm_agent` 的 `call_mcp_tool` / `read_mcp_result`） | 支持，**只有零额度证据** | 默认只给别的机器上的地址（HTTP），本机跑的（程序、`127.0.0.1`）要 Agent 的 `[agent_mcp] local` 点名；workspace 的 `agent_tools` 默认含 `mcp_servers`（[使用说明](usage.md#agent-机器上的-mcp-server)、[配置说明](configuration.md#agent_mcp)、[P50 记录](research/p50-agent-mcp-2026-09-22.md)）。HTTP 经 Agent 上的 `curl`。**验过的**：Rust 单元测试（默认只给远端地址、其余写明原因、配置里关掉的不给、点名的本机程序调通且配置 `env` 到了、本机程序留在进程组里的子进程随它关掉（P52）、长结果分段读全、`curl` 带会话号和请求头、从一直开着的 SSE 流里取到回复就停、401 报要登录、连不上报连不上、key 只在只有本账号能读的文件里）；中立 MCP 客户端对真实二进制（stdio 和 HTTP 都调通、52 000 字节经 `read_mcp_result` 读全、Agent 继承来的登录变量没到 server、会话结束 server 进程没了）；**真实 Claude Code 2.1.278、Codex 0.154.0 与 0.155.1（`gpt-5.1-codex` 和不写模型的 Code Mode）接上真实的 `ccnm internal agent-skills`**（假模型，零额度，toexec `evidence/v4-mcp/agent-mcp/`）；**真实的 DeepWiki 经 `curl` 连通**（本机开着 HTTPS 代理），它 407 KB 的 `read_wiki_contents` 分 13 段一个字节不少地读回来。**真机验过了**（2026-09-23，[P50 记录的补验一节](research/p50-agent-mcp-2026-09-22.md#2026-09-23-补验)）：fodelf 上的真实 Claude Code 2.1.272（`claude-opus-5`）经双机 SSH 链路，没人提示就用 Agent 上的 DeepWiki 答了一个问题，凭据是 fodelf 上 `ccnm_agent` 起的 5 个 `curl`，不是模型自述；6 个真实的 OAuth 地址都报"需要登录"；Linux（Debian 13 / x86_64，curl 8.14.1）上中立客户端 4 条全过、真实 DeepWiki 读全；Codex 0.154.0 / 0.155.1 只把 `HOME`、`PATH` 等 4 个变量交给 `ccnm_agent`，用到别的 `${VAR}` 的 server 写明缺哪个、不起。那一轮查出 `curl` 的请求文件在会话结束后留在 Agent 的 `$TMPDIR` 里（Claude Code 退出时强杀 MCP server、不关 stdin），已改成用完就删。**没验的**：Codex 当 Agent 的受管会话；真实模型读超过 32 KiB 的结果；`local` 点名的本机 server 在真机上；`~/.claude.json` 里的 server 调用超时调不了，慢网络上大结果会撞 60 秒。**故意不做的**：把这些 server 直接写进 Claude / Codex 的配置（实测大结果会丢，见记录）；老的 HTTP+SSE 传输；server 的 resources / prompts。 |
