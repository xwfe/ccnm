...
| Codex Agent Instance，exec-server 链（workspace 写 `codex_exec_server = true`） | **离线验证，未真机**（P21–P23 完成，P24 待授权） | Codex 用自带的执行工具，由 Runtime 上受 ccnm 监督和过滤的官方 `codex exec-server` 执行；只开交互模式，print 在创建会话前拒绝；Claude 不受影响。本机零额度闭环：真实 Codex 0.154.0 + 真实 `exec-serve` + 真实 exec-server，读、改、跑命令通过，批准提权后的越界命令和 patch 被 ccnm 拒绝且不落盘，传输断线后不重连、不 resume，Runtime 连不上时 Codex 没有任何执行工具（[P23 记录](research/p23-stdio-transport-2026-09-16.md)）。**边界**：ssh 那一跳用本机 TCP 管道代替；Runtime 只在 macOS 上接过真 exec-server（Linux 只在容器里测过 exec-server 本身）；没有真实模型回合；两台机器上装着的 ccnm 还没有这条链。 |
