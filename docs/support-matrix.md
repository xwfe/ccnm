...
| Runtime MCP relay（P49） | stdio 转接已实现，有本机测试及零额度 Host 证据 | 同组后代 P52 起在交权前清掉并确认，清不掉不交权（macOS 已验、**Linux 未跑**）；离开进程组的后代够不着；仍缺完整真实模型组合验收 |
...
**C51-01**（relay server 退出后同组子进程仍写、写锁却交出）：P52 已修，macOS 已验证，**Linux 回归未跑**，见 [P52 记录](research/2026-09-25-p52-relay-group-cleanup.md)。在 Linux Runtime 上跑完之前，或 server 会派生离开进程组的后代（`setsid`、守护进程）时，要求可靠单写交接仍先停用 `[runtime_mcp]`，关闭现有会话并核实旧进程树；仅改配置不能清理旧进程，删除 guard 不能证明可以安全重开。
